...
const seedCandidates: Candidate[] = [
  ...
  },
  {
    ...
    stage: "Shortlisted",
    fitScore: 79,
    notes: "Good portfolio. Needs speed + crew management test.",
  },
  {
    id: 103,
    name: "Manav Trivedi",
    ...
    stage: "Trial Event",
    fitScore: 91,
    notes: "Reliable. Verify vendor negotiation and overtime handling.",
  },
];
...
export default function HRPage() {
  ...
  useEffect(() => {
    ...
    setTeam(
      (savedTeam && savedTeam.length ? savedTeam : seedTeam).map((m) => ({
        ...m,
    ...
    setCandidates(
      (savedCandidates && savedCandidates.length ? savedCandidates : seedCandidates).map(
        (c) => ({
          ...c,
    ...
    setTraining(
      (savedTraining && savedTraining.length ? savedTraining : seedTraining).map(
        (t) => ({
          ...t,
    ...
    setAudit(savedAudit && savedAudit.length ? savedAudit : []);
  }, []);
  ...
  const filteredTeam = useMemo(() => {
    ...
    return team
      ...
      .sort((a, b) => {
        // Active first, then workload high
  ...
  const upsertMember = (payload: TeamMember) => {
    ...
    setTeam((prev) => {
      ...
      return [
        { ...payload, createdAt: ts, updatedAt: ts },
        ...prev,
      ].sort((a, b) => b.id - a.id);
    });
  ...
  const exportJSON = () => {
    ...
    a.download = `eventura-hr-export-${new Date()
      .toISOString()
      .slice(0, 10)}.json`;
    a.click();
  ...
  const resetToSeed = () => {
    if (!isCEO) return;
    const ok = window.confirm("Reset HR to seed data? This overwrites current HR DB.");
    if (!ok) return;
    ...
    pushAudit("RESET", "Reset HR database to seed data");
  };
          ...
          <div className="eventura-header-row">
            <div>
              ...
              <p className="eventura-subtitle">
                Enterprise HR dashboard for Eventura: directory, capacity, hiring,
                training, audit log.
              </p>
            </div>
          ...
          <section className="eventura-panel" style={{ marginBottom: "0.9rem" }}>
            <div className="eventura-actions" style={{ display: "flex", gap: "0.6rem", flexWrap: "wrap" }}>
              ...
              <button
                className="eventura-button-secondary"
                disabled={!isCEO}
                title={!isCEO ? "CEO only" : "Reset to seed data"}
                onClick={resetToSeed}
              >
                ♻ Reset
              </button>
...
function HROverviewView({ summary, team }: { summary: HRSummary; team: TeamMember[] }) {
  ...
  const roleHeat = roles.map((role) => {
    ...
    const avg = members.reduce((sum, m) => sum + (m.workload || 0), 0) / members.length;
      ...
      <section className="eventura-columns">
        ...
        <div className="eventura-panel">
          ...
          <ul className="eventura-bullets">
            ...
            {summary.roleCounts["Event Manager"] < 2 && (
              <li><span className="eventura-tag eventura-tag-blue">Hiring suggestion</span> Add 1 Event Manager in next 6–9 months.</li>
            )}
            {summary.roleCounts["Decor Specialist"] < 1 && (
              <li><span className="eventura-tag eventura-tag-amber">Critical</span> Need at least 1 Decor Specialist core.</li>
            )}
            {summary.roleCounts["Logistics"] < 1 && (
              <li><span className="eventura-tag eventura-tag-amber">Critical</span> Need logistics core for scale.</li>
            )}
          </ul>
  ...
  return (
    <section className="eventura-columns">
      ...
      <div className="eventura-panel">
          ...
          <table className="eventura-table">
            ...
            <tbody>
              {Object.entries(coreByRole).map(([role, count]) => {
                ...
                if (role === "Logistics" && count < 1) comment = "Need strong logistics for scale.";
                return (
                  <tr key={role}>
                    <td>{role}</td>
                    <td>{count}</td>
        ...
        <ul className="eventura-bullets">
          <li>Decor freelancers: {team.filter((m) => m.status === "Freelancer" && m.role === "Decor Specialist").length}</li>
          <li>Logistics freelancers: {team.filter((m) => m.status === "Freelancer" && m.role === "Logistics").length}</li>
        </ul>
      </div>
    </section>
  );
  ...
  return (
    <section className="eventura-columns">
      ...
      <div className="eventura-panel">
        ...
        <p className="eventura-small-text">
          Add prompts like: “who is overloaded?”, “who is ready for promotion?”, “who needs training first?”
        </p>
      </div>
    </section>
  );
...
function HRAuditView({ audit }: { audit: AuditItem[] }) {
  return (
    <section className="eventura-panel">
      ...
      <p className="eventura-small-text">Tracks HR changes (enterprise style). Latest first.</p>
            ...
            {audit.length === 0 && (
              <tr>
                <td colSpan={4} style={{ color: "#9ca3af" }}>
                  No actions yet. Add/edit/delete a member as CEO to see audit entries.
                </td>
              </tr>
            )}
          </tbody>
        ...
        <div className="eventura-header-row">
          <div>
            ...
            <p className="eventura-small-text" style={{ marginTop: 6 }}>
              {canEdit ? "Changes auto-save into local HR database." : "Read-only"}
            </p>
          </div>
